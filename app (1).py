...
def load_data():
    try:
        ...
        return model, X_train, df, num_cols, cat_cols, target

    except Exception as e:
        st.error(f"Setup Error: {e}")
        return None, None, None, [], [], None

model_pipeline, X_train, df_full, num_cols, cat_cols, target_col = load_data()

# ==========================================
# 2. LOGIC: SMART & MEDICALLY ACCURATE
# ==========================================
...
def generate_shap(model, X_train, instance, role, pred_class):
    try:
        ...
        if role == "Doctor 🩺":
            return "### 🔍 Root Cause Analysis (SHAP)", df_imp[['Feature', 'Impact']]
        else:
            # --- DYNAMIC TEXT BASED ON RISK ---
            if pred_class == 1:
                txt = "### ⚠️ Why is my risk high?\nThese factors are driving the risk UP:\n"
            else:
                txt = "### ✅ Why is my risk low?\nGreat job! These factors are keeping you healthy:\n"
        ...
        return f"Could not generate chart: {e}", None
...
def generate_dice(model, df, instance, target, role):
    try:
        ...
        # --- 🧠 SMART LOGIC: MEDICAL CONSISTENCY ---

        # 1. Ignore "Age" changes
        if 'age' in new_vals:
            new_vals['age'] = original['age']

        # 2. Link GFR -> Creatinine & BUN
        is_gfr_improved = (new_vals.get('gfr', 0) > 60) and (original.get('gfr', 0) < 60)

        if is_gfr_improved:
            # Force Creatinine to healthy range (~1.1)
            for col in ['creatinine', 'sc', 'serum_creatinine']:
                for c in df.columns:
                    if col in c: new_vals[c] = 1.1

            # Force BUN to healthy range (~18)
            for col in ['bun', 'bu', 'urea']:
                for c in df.columns:
        ...
        if role == "Doctor 🩺":
            if not changes: return "No modifiable risk factors found.", None
            df_changes = pd.DataFrame(changes)
            txt = "### 📋 Recommended Adjustments\n*Note: Creatinine/BUN values harmonized with GFR target.*"
            return txt, df_changes
        else:
            if not changes: return "Maintain a healthy lifestyle.", None
            txt = "### ✅ Action Plan\n"
            for c in changes:
                direction = "Lower" if c['Target'] < c['Current'] else "Increase"
                clean_name = c['Feature'].replace('_', ' ').capitalize()
                txt += f"- **{direction} {clean_name}**: {c['Advice']}\n"
            return txt, None
...
# 3. UI
# ==========================================
st.set_page_config(layout="wide", page_title="Kidney AI")
col_h1, col_h2 = st.columns([3, 1])
with col_h1: st.title("🩺 Kidney Health AI")
with col_h2: role = st.selectbox("View as:", ["Patient 👤", "Doctor 🩺"])

st.sidebar.header("Patient Data")
if df_full is not None:
    # Quick Buttons
    if st.sidebar.button("🎲 Load Random Patient"):
    ...
    # --- CATEGORICAL INPUTS (FIXED CRASH) ---
    for col in cat_cols:
        # Convert options to string to prevent TypeError
...
with col2:
    if st.button("Run Diagnostics", type="primary", use_container_width=True):
        ...
        st.session_state['messages'] = [{"role": "ai", "content": "Analysis Complete. Use the buttons below."}]

    if 'result' in st.session_state:
        pred, prob = st.session_state['result']
        if pred == 1:
            st.error(f"⚠️ High Risk Detected (Confidence: {prob:.1%})")
        else:
            st.success(f"✅ Low Risk (Confidence: {(1-prob):.1%})")

        c_btn1, c_btn2 = st.columns(2)

        # --- DYNAMIC BUTTON LABELS ---
        label_why = "❓ Why High?" if pred == 1 else "❓ Why Low?"
        label_how = "💊 How to Fix?" if pred == 1 else "💎 How to Maintain?"

        ask_why = c_btn1.button(label_why)
        ask_how = c_btn2.button(label_how)

        if "messages" not in st.session_state: st.session_state.messages = []

        if ask_why:
            # Pass prediction to generate correct text
            msg, data = generate_shap(model_pipeline, X_train, input_df, role, pred)
            user_q = "Why is my risk high?" if pred == 1 else "Why is my risk low?"
        ...
        if ask_how:
            ...
            user_q = "How do I fix this?" if pred == 1 else "How do I maintain this?"
            st.session_state.messages.append({"role": "user", "content": user_q})
...
def load_data():
    try:
        ...
        # --- 🔧 FIX MISSING VALUES (CRITICAL FOR DiCE) ---
        for col in num_cols:
            if X[col].isnull().any():
...
# ==========================================
# 2. LOGIC: SMART ANALYSIS & AUDIT TOOLS
# ==========================================
...
def generate_dice(model, df, instance, target, role):
    try:
        ...
        if role == "Doctor 🩺":
            ...
            return "### 📋 Recommended Adjustments", pd.DataFrame(changes)
        else:
...
col_h1, col_h2 = st.columns([3, 1])
with col_h1: st.title("🩺 Kidney Health AI System")
with col_h2: role = st.selectbox("View as:", ["Patient 👤", "Doctor 🩺", "Regulator ⚖️"])

# ==========================================
# VIEW: REGULATOR / AUDITOR (DASHBOARD STYLE)
# ==========================================
if role == "Regulator ⚖️":
    ...
    with st.container(border=True):
        st.subheader("1. System Health Metrics")
        k1, k2, k3, k4, k5 = st.columns(5)
        k1.metric("Training Accuracy", f"{metrics['train_acc']:.1%}", help="Check Overfitting")
        k2.metric("Testing Accuracy", f"{metrics['test_acc']:.1%}", delta=f"{metrics['test_acc'] - metrics['train_acc']:.1%}", delta_color="inverse")
        k3.metric("Recall (Safety)", f"{metrics['recall']:.2%}", help="% of actual sick patients found")
        k4.metric("Precision", f"{metrics['precision']:.2%}", help="False alarm rate")
        k5.metric("F1 Score", f"{metrics['f1']:.2f}", help="Overall Balance")
    ...
    with col_right:
        with st.container(border=True):
            ...
            st.success(f"✅ **Logic Check:** Top Driver is **'{top_feature}'**.")

    # --- ROW 3: SAFETY STRESS TEST ---
    with st.container(border=True):
        st.subheader("4. Safety Protocol Stress Test (DiCE)")
        s1, s2 = st.columns([1, 3])
        run_test = s1.button("🚨 Run Live Stress Test", use_container_width=True)
        ...
        if run_test:
            with st.spinner("Auditing optimization constraints..."):
                ...
                for i in range(len(sample)):
                    ...
                    results.append({"Patient ID": i+1, "Changes": changes, "Age Violation": "⚠️ FAIL" if age_changed else "✅ PASS"})
                ...
                with s2:
                    ...
                    m3.metric("Status", "✅ PASSED" if violations == 0 else "❌ FAILED")
                    st.dataframe(pd.DataFrame(results), use_container_width=True, hide_index=True)
        else:
            with s2: st.info("Click 'Run Live Stress Test' to audit the counterfactual engine.")

    # --- ROW 4: INTERACTIVE CHAT ---
    st.markdown("---")
    with st.expander("💬 Open Interactive Auditor Terminal", expanded=True):
...
# ==========================================
    ...
    with col2:
        ...
        if 'result' in st.session_state:
            pred, prob = st.session_state['result']
            if pred == 1: st.error(f"⚠️ High Risk Detected (Confidence: {prob:.1%})")
            else: st.success(f"✅ Low Risk (Confidence: {(1-prob):.1%})")
